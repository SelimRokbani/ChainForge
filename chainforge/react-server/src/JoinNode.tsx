  ...
  const groupAndJoinByVars = useCallback(
    (
      ...
      if (varnames.length === 0) {
        ...
        // ENHANCED: Merge all metadata from all items in the group
        const mergedFillHistory: Dict<any> = {};
        const mergedMetavars: Dict<any> = {};

        // Collect all possible tabular column names we want to preserve
        ...
        // First pass: Collect all tabular data to ensure it's not lost
        const allTabularData: Dict<any> = {};
        items.forEach((item) => {
          if (typeof item !== "string") {
            // Check both fill_history and metavars for tabular data columns
            [item.fill_history || {}, item.metavars || {}].forEach((source) => {
              Object.entries(source).forEach(([key, value]) => {
                // Prioritize known tabular columns or any column that might be part of tabular data
                if (
                  tabularColumns.includes(key) ||
                  (typeof value === "string" && value.length < 1000)
                ) {
                  allTabularData[key] = value;
                }
              });
            });
          }
        ...
        // Now collect regular metadata, but give precedence to the tabular data
        items.forEach((item) => {
          if (typeof item !== "string") {
            // Merge fill_history (vars)
            if (item.fill_history) {
              Object.entries(item.fill_history).forEach(([key, value]) => {
                mergedFillHistory[key] = value;
              });
            }

            // Merge metavars
            if (item.metavars) {
              Object.entries(item.metavars).forEach(([key, value]) => {
                mergedMetavars[key] = value;
              });
            }
          }
        });

        // Ensure all tabular data is in both metavars and fill_history
        Object.entries(allTabularData).forEach(([key, value]) => {
          mergedMetavars[key] = value;
          mergedFillHistory[key] = value;
        });
        ...
        return [
          {
            text: joined_text,
            fill_history: mergedFillHistory,
            metavars: mergedMetavars,
            llm,
            uid: uuid(),
          },
        ];
