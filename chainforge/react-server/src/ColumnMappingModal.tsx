...
export interface ColumnMappingModalRef {
  openModal: (detectedColumns: string[], onConfirm: (mappings: ColumnMapping[]) => void) => void;
}
...
const ColumnMappingModal = forwardRef<ColumnMappingModalRef, ColumnMappingModalProps>((props, ref) => {
  const [isOpen, setIsOpen] = useState(false);
  const [columns, setColumns] = useState<ColumnMapping[]>([]);
  const [onConfirmCallback, setOnConfirmCallback] = useState<((mappings: ColumnMapping[]) => void) | null>(null);
  
  // Function to open the modal with detected column names
  const openModal = (detectedColumns: string[], onConfirm: (mappings: ColumnMapping[]) => void) => {
  ...
  // Handle input change
  const handleColumnChange = (index: number, field: 'responseField' | 'displayName', value: string) => {
  ...
  return (
    ...
    >
      <Stack spacing="md">
        <Alert icon={<IconAlertCircle size="1rem" />} title="Column Mapping" color="blue">
          Map response data fields to display names. This affects how evaluation results are displayed.
        </Alert>
        ...
        {columns.map((column, index) => (
          <Group key={index} position="apart" grow>
            <TextInput
              ...
              onChange={(e) => handleColumnChange(index, 'responseField', e.target.value)}
